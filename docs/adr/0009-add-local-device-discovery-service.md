...
The service validates the request `Origin` against a configured allowlist (the business system's origin) and rejects all other callers. This keeps device identity from being harvested by arbitrary web pages opened in the user's browser. Binding to `127.0.0.1` ensures the service is not reachable from other machines.
